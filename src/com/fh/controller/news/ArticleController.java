...
import com.fh.service.news.ArticleService;
import com.fh.service.news.ArticleTypeService;
import com.fh.util.AppUtil;
import com.fh.util.ClobUtil;
...
public class ArticleController extends BaseController {
	...
	public ModelAndView list(Page page){
		...
		try{
			...
			mv.setViewName("news/article/article_list");
	...
	public ModelAndView goAdd() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		List<PageData> pt = articletypeService.listAll(pd);
		try {
			mv.setViewName("news/article/article_edit");
			mv.addObject("typeList",pt);
	...
	public ModelAndView goEdit(){
		...
		try {
			...
			mv.setViewName("news/article/article_edit");
			mv.addObject("typeList",articletypeService.listAll(pd));
	...
	public ModelAndView exportExcel(){
		...
		try{
			...
			for(int i=0;i<varOList.size();i++){
				...
				vpd.put("var7", varOList.get(i).getString("TYPEID"));	//7
				vpd.put("var8", varOList.get(i).getString("TITLE"));	//8
				vpd.put("var9", varOList.get(i).get("CLICK").toString());	//9
				vpd.put("var10", varOList.get(i).getString("CONTENT"));	//10
				vpd.put("var11", varOList.get(i).getString("TYPENAME"));	//11
				vpd.put("var12", varOList.get(i).getString("LANGUAGE"));	//12
				vpd.put("var13", varOList.get(i).getString("KEYWORD"));	//13
				varList.add(vpd);
			}
