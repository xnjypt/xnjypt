package com.fh.controller.user.loginmember;
...
/** 
 * 类名称：LoginMemberController
 * 创建人：FH 
 * 创建时间：2016-10-28
 */
@Controller
@RequestMapping(value="/loginmember")
public class LoginMemberController extends BaseController {
	
	String menuUrl = "loginmember/list.do"; //菜单地址(权限用)
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, "新增LoginMember");
	...
	public void delete(PrintWriter out){
		logBefore(logger, "删除LoginMember");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			loginmemberService.delete(pd);
			out.write("success");
			out.close();
		} catch(Exception e){
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, "修改LoginMember");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		loginmemberService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView list(Page page){
		logBefore(logger, "列表LoginMember");
		...
		try{
			pd = this.getPageData();
			page.setPd(pd);
			List<PageData>	varList = loginmemberService.list(page);	//列出LoginMember列表
			mv.setViewName("member/loginmember/loginmember_list");
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增LoginMember页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			mv.setViewName("member/loginmember/loginmember_edit");
	...
	public ModelAndView goEdit(){
		logBefore(logger, "去修改LoginMember页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			pd = loginmemberService.findById(pd);	//根据ID读取
			mv.setViewName("member/loginmember/loginmember_edit");
	...
	public Object deleteAll() {
		logBefore(logger, "批量删除LoginMember");
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				loginmemberService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
	...
	public ModelAndView exportExcel(){
		logBefore(logger, "导出LoginMember到excel");
		...
		try{
			...
			List<PageData> varOList = loginmemberService.listAll(pd);
