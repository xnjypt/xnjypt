package com.fh.controller.money.xnboperationrecord;
...
/** 
 * 类名称：XnboperationrecordController
 * 创建人：FH 
 * 创建时间：2016-10-28
 */
@Controller
@RequestMapping(value="/xnboperationrecord")
public class XnboperationrecordController extends BaseController {
	
	String menuUrl = "xnboperationrecord/list.do"; //菜单地址(权限用)
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, "新增Xnboperationrecord");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("XNBOPERATIONRECORD_ID", this.get32UUID());	//主键
		...
		pd.put("TOPUPADDRESS", "");	//充值地址
		pd.put("TRADINGID", "");	//交易ID
		xnboperationrecordService.save(pd);
	...
	public void delete(PrintWriter out){
		logBefore(logger, "删除Xnboperationrecord");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			xnboperationrecordService.delete(pd);
			out.write("success");
			out.close();
		} catch(Exception e){
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, "修改Xnboperationrecord");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		xnboperationrecordService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView list(Page page){
		logBefore(logger, "列表Xnboperationrecord");
...
//			}
			page.setPd(pd);
			List<PageData>	varList = xnboperationrecordService.list(page);	//列出Xnboperationrecord列表
			mv.setViewName("money/xnboperationrecord/xnboperationrecord_list");
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增Xnboperationrecord页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			mv.setViewName("money/xnboperationrecord/xnboperationrecord_edit");
	...
	public ModelAndView goEdit(){
		logBefore(logger, "去修改Xnboperationrecord页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			pd = xnboperationrecordService.findById(pd);	//根据ID读取
			mv.setViewName("money/xnboperationrecord/xnboperationrecord_edit");
	...
	public Object deleteAll() {
		logBefore(logger, "批量删除Xnboperationrecord");
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				xnboperationrecordService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
	...
	public ModelAndView exportExcel(){
		logBefore(logger, "导出Xnboperationrecord到excel");
		...
		try{
			...
			List<PageData> varOList = xnboperationrecordService.listAll(pd);
			...
			for(int i=0;i<varOList.size();i++){
				...
				vpd.put("var15", varOList.get(i).getString("TOPUPADDRESS"));	//15
				vpd.put("var16", varOList.get(i).getString("TRADINGID"));	//16
				varList.add(vpd);
			}
