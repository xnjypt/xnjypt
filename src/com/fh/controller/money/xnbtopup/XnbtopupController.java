package com.fh.controller.money.xnbtopup;
...
import com.fh.service.money.xnbtopup.XnbtopupService;

/** 
 * 类名称：XnbtopupController
 * 创建人：FH 
 * 创建时间：2016-10-28
 */
@Controller
@RequestMapping(value="/xnbtopup")
public class XnbtopupController extends BaseController {
	
	String menuUrl = "xnbtopup/list.do"; //菜单地址(权限用)
	@Resource(name="xnbtopupService")
	private XnbtopupService xnbtopupService;
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, "新增Xnbtopup");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("XNBTOPUP_ID", this.get32UUID());	//主键
	...
	public void delete(PrintWriter out){
		logBefore(logger, "删除Xnbtopup");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			xnbtopupService.delete(pd);
			out.write("success");
			out.close();
		} catch(Exception e){
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, "修改Xnbtopup");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		xnbtopupService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
...
//			}
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增Xnbtopup页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			mv.setViewName("money/xnbtopup/xnbtopup_edit");
	...
	public ModelAndView goEdit(){
		logBefore(logger, "去修改Xnbtopup页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			pd = xnbtopupService.findById(pd);	//根据ID读取
			mv.setViewName("money/xnbtopup/xnbtopup_edit");
	...
	public Object deleteAll() {
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				xnbtopupService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
	...
	public ModelAndView exportExcel(){
		logBefore(logger, "导出Xnbtopup到excel");
		...
		try{
			...
			List<PageData> varOList = xnbtopupService.listAll(pd);
