package com.fh.controller.money.userxnb;
...
/** 
 * 类名称：UserxnbController
 * 创建人：FH 
 * 创建时间：2016-10-28
 */
@Controller
@RequestMapping(value="/userxnb")
public class UserxnbController extends BaseController {
	
	String menuUrl = "userxnb/list.do"; //菜单地址(权限用)
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, "新增Userxnb");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("USERXNB_ID", this.get32UUID());	//主键
		...
		pd.put("USERALLNUM", "");	//会员总数量
		pd.put("USERFREEZENUM", "");	//冻结数量
		pd.put("XNBTYPE", "");	//虚拟币类型
		userxnbService.save(pd);
	...
	public void delete(PrintWriter out){
		logBefore(logger, "删除Userxnb");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			userxnbService.delete(pd);
			out.write("success");
			out.close();
		} catch(Exception e){
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, "修改Userxnb");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		userxnbService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
...
//			}
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增Userxnb页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			mv.setViewName("money/userxnb/userxnb_edit");
	...
	public ModelAndView goEdit(){
		logBefore(logger, "去修改Userxnb页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			pd = userxnbService.findById(pd);	//根据ID读取
			mv.setViewName("money/userxnb/userxnb_edit");
	...
	public Object deleteAll() {
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				userxnbService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
	...
	public ModelAndView exportExcel(){
		logBefore(logger, "导出Userxnb到excel");
		...
		try{
			...
			List<PageData> varOList = userxnbService.listAll(pd);
			...
			for(int i=0;i<varOList.size();i++){
				...
				vpd.put("var11", varOList.get(i).get("USERALLNUM").toString());	//11
				vpd.put("var12", varOList.get(i).get("USERFREEZENUM").toString());	//12
				vpd.put("var13", varOList.get(i).getString("XNBTYPE"));	//13
				varList.add(vpd);
			}
